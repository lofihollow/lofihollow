                ...
                if (kv.Value.CurrentStage < kv.Value.Stages.Count) {
                    if (kv.Value.Stages[kv.Value.CurrentStage].CollectItemFullName != "") {
                        for (int i = 0; i < GameLoop.World.Player.player.Inventory.Length; i++) {
                            if (GameLoop.World.Player.player.Inventory[i].FullName() == kv.Value.Stages[kv.Value.CurrentStage].CollectItemFullName) {
                                if (GameLoop.World.Player.player.Inventory[i].Quality >= kv.Value.Stages[kv.Value.CurrentStage].CollectItemQuality) {
                                    if (GameLoop.World.Player.player.Inventory[i].ItemQuantity >= kv.Value.Stages[kv.Value.CurrentStage].CollectItemQty) {
